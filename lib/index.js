...
Nikko.prototype.registerPlugin = function(plugin){
    ...
    var plugInfo = {};
    var pluginPkg = require(path.join(this.pluginPath, plugin, 'package.json'));
    console.log('PACKAGE INFO', pluginPkg);

    plugInfo.name = pluginPkg.name;

    //TODO: we should add a custom entry on package: nikko-command
    // create [name]
    plugInfo.description = [pluginPkg.name.replace('nikko-', ''), pluginPkg.description];
    json.plugins.push(plugInfo);
    ...
    this.emit('plugin.registered.' + plugin, {plugin:plugin});
    return this;
};
